...
export default function ManualRiskOverridePanel({
  ...
  const [inputs, setInputs] = useState<ManualInputs>(initialInputs);

  const result = useMemo(() => {
    const factors = snapshot.factors.map((factor) => {
      const rawValue = parseInput(inputs[factor.id], factor.rawValue);
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            官方模型默认使用上一交易日收盘数据。你也可以把自己查到的最新值填入下方入口，系统会在当前浏览器内即时计算新的 Z 值、市场温度和定投倍率。
          </p>
        </div>
